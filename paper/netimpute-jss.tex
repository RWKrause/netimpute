...
paper introduces \pkg{netimpute}, an \proglang{R} package that (a) computes a
broad battery of node-level structural and attribute
homophily measures across one or more networks, (b) can reduce these to a
manageable set of principal-component predictors while optionally preserving
specific raw measures that are themselves part of substantive hypotheses,
(c) provides a dyadic (cell-level) regression for network ties in the
spirit of MR-QAP \citep{Krackhardt1988}, and (d) combines both into \code{netmice()}, a chained-equations
...
Two kinds of missingness arise in studies involving networks, which are
usually
handled by different tools. Missing values on \emph{node attributes}
...
\citep{Rubin1987,LittleRubin2002} or model based likelihood treatments (e.g., FIML). Missing \emph{network ties} are instead the subject of a smaller,
...
In practice, these two kinds of missingness are rarely independent
...
(e.g., do two students share a gender or interest?)
...
building the dyadic covariates used to impute ties. Any imputation focused only on one side of this problem works only conditional on the other missing data being irrelevant.

\pkg{netimpute} addresses this by combining the imputation of attributes and network ties together in one function. While the imputation of
...
missing data: both assume a fully observed network. The same is true for most functions in the new \pkg{manynet} package \citep{manynet} and its sister packages.
...
by \pkg{netimpute} as the engine for the actual univariate imputation
step. By default it uses predictive mean matching for numeric and binary targets,
with \pkg{mice}'s other numeric-response univariate methods selectable
through the same \code{method} argument and multinomial
...
proposed imputation procedures based on the observed structure. These
procedures are
not implemented as a general-purpose, formula-extensible \proglang{R}
packages; \pkg{netimpute}'s dyadic regression step can be seen as a simple, point-prediction analogue of MR-QAP
\citep{Krackhardt1988} used specifically to build a predictive (rather than
inferential, and thus avoiding the need for permutation) model for tie imputation. The default tie-wise updating scheme
...
restricted to the missing cells, so observed ties are never resampled. This is not seen as a replacement or improvement upon proper ERGM-based (or SAOM-based) imputations, but rather as a fast, scalable, and easy to use way to capture the essential dependencies between ties, attributes, and networks for the purpose of imputation.
...
A network carries a lot of information that can be useful for imputing not just
itself but also missing attribute values. The function
...
together with, for every \code{attribute}, a
...
holes measures, coreness, and, if \pkg{sna} is installed, six
additional measures) using exactly the same homophily block.

Both functions are wrappers around  \code{net\_measures(net, attributes, measure\_set)}: \code{measure\_set}
...
\code{netmice()} help pages. The same \code{measure\_set} argument is
...
error and it is currently recommended to split signed networks into two non-negative networks (e.g., ``friendship'' and
``antagonism'') for separate imputation, and recombine them afterwards
(Section~\ref{sec:discussion}). For non-negative weighted networks, two
measures are redefined:
reciprocity of a node becomes the average $|w_{ij} - w_{ji}|$ over alters tied in
either direction, and local clustering becomes the geometric-mean
...
the  binary clustering coefficient when all weights equal $1$.
...
of attribute data frames), stacks the results, and
offers three output modes: \code{output = "measures"} returns the stacked
raw measures. \code{output = "pca"} instead returns as many leading principal components of
the measure matrix as the \code{PCA} budget allows, addressing the multicollinearity that is
otherwise inevitable once a dozen or more structural and homophily measures
are computed per network and the inflation of predictors, which can quickly exceed the $N$ in small networks. \code{output = "both"} returns the principal
...
not want diluted by including it
into a composite component, which can also be resiudalised on the remaining predictors so the
components capture variance orthogonal to it (\code{residualize\_kept = TRUE}) .

\subsubsection{The predictor budget}
\label{sec:pca}

How many predictors any single imputation model may carry is governed by one
argument, \code{PCA}, a list with \code{n} (a fixed ceiling) and/or
\code{ratio} (observations required per predictor); when both are given the
smaller budget binds. Whatever the budget, terms that must keep their own
named coefficient --- a network's own \code{reciprocity} and \code{twopath},
the cross-network cell terms, the \code{isolate} indicator, and anything named
in \code{models} --- are exempt, and the budget applies to the remainder.

The denominator of \code{ratio} is deliberately not the row count. For a
binary target, which includes every tie model, it is the \emph{minority class
count} among the observed cells. A logistic model is limited by its rarer
outcome rather than by its sample size: a 30-node network at density $0.15$
presents roughly $870$ dyadic rows but only about $130$ ties, so budgeting
against the rows would licence enough predictors to separate the data
perfectly. The default \code{ratio = 10} is the conventional
events-per-variable floor \citep{Peduzzi1996,Harrell2015}. Continuous and
multinomial targets use the observed row count.

\subsection{Dyadic regression}
\label{sec:dyad}

\code{dyad\_regression()} vectorises a target network's adjacency matrix
(excluding the diagonal) and regresses it for every node attribute on an
ego value, an alter value, and either an absolute difference (continuous
attributes) or a same-category indicator (categorical attributes), reciprocity
(the transposed target network), a bounded $0/1$ two-path indicator
(\code{twopath} --- ``at least one shared contact'', i.e.\ at least one
path $i \to k \to j$ to impute $x_{ij}$), and, for every
\emph{other} network supplied, its tie value, its reciprocity (transpose), and
the sender's and the receiver's out- and in-degree in that network. The endogenous terms are configurable through
\code{net\_endo\_terms}: besides \code{reciprocity} and \code{twopath}, the
geometrically weighted change statistics \code{gwesp}, \code{gwodegree} and
\code{gwidegree} are available for binary targets. These are ERGM change
statistics --- the movement in the corresponding network statistic if the
cell went from $0$ to $1$, evaluated on the network with that cell removed
--- so the degree terms are leave-one-out by construction and the tie being
imputed never enters its own predictor. They are not enabled by default:
\code{twopath} is bounded by $1$, whereas \code{gwesp}'s change statistic is
bounded by $e^{\alpha} + 2(n-2)$, and in our diagnostics a positive
coefficient on it generated substantially too many ties on sparse networks.
An undirected target carries no \code{reciprocity} term, since there
$x_{ji}$ \emph{is} $x_{ij}$. When
...
principal components. By default the dyad-level cross-network terms (the other
network's cell and its transpose) are kept as predictors and not taken into the
PCA to preserve the direct relationship between the networks. The complete list
of term names, as referenced from
...
impossible; argument \code{structural}) are removed internally from the dyad data
entirely, like the diagonal. The model is fit by \code{glm()} with a user-specified
\code{family} (default Gaussian). Alternatively, \code{random\_intercepts} fits the model
...
target's observed values and, like \code{mice::quickpred()}, with its missingness
...
\code{reciprocity} and \code{twopath} (and any other active endogenous
term) always included unscreened.
...
target's model. Finally, near-duplicate survivors are pruned
...
endogenous predictors exempt from pruning. The result
...
Section~\ref{sec:netmice} with target-specific models in which every
...
else with missing data that is not selected as a (recursive) predictor, target, or explicitly included in an imputation model is
dropped from the imputation algorithm entirely, saving computation time and avoiding that the model gets flooded with irrelevat predictors.

\subsection{Joint imputation}
\label{sec:netmice}

\code{netmice()} is a chained-equations routine, architecturally modelled on
...
Each chain starts from a conservative state: missing attribute
...
very first sweep's structural terms downward, but avoids seeding a
self-reinforcing surplus of random initial ties that later sweeps amplify. A density-based initial imputation is available as \code{net\_init = "sample"}.
...
is fit once per visit --- logistic regression for a
...
values of the endogenous statistics. A binary tie is then drawn from
...
estimated by pseudo-likelihood restricted to the missing cells. It yields properly dependent within-network
imputations and avoids potential synchronous-update artifacts of the alternative
scheme, \code{net\_update = "simultaneous"}, in which one model
per visit imputes every missing cell at once from the same snapshot
of the statistics (e.g., if both $x_{ij}$ and $x_{ji}$ are missing, their imputation would be independent of each other).

\paragraph{Undirected networks.} In an undirected network the cells
$x_{ij}$ and $x_{ji}$ are not two ties but two records of the same tie, and
imputing them independently would return a matrix that is no longer
symmetric. This is more than an aesthetic defect: throughout the package
directedness is detected from the matrix itself, so a single desymmetrized
pair reclassifies the network as directed for every node- and dyad-level
measure derived from it in the remaining sweeps, and in the returned
object. A network is therefore treated as undirected whenever the supplied
matrix is symmetric in its values \emph{and} in its pattern of missing
cells --- precisely the case in which the two cells record one tie --- and
is then imputed one tie at a time.

This classification is established once, from the incomplete data supplied
to \code{netmice()}, and is thereafter used wherever directedness matters:
the tie updater, the initialisation, the node-level measures that serve as
attribute predictors, and the per-iteration diagnostics. It deliberately is
not re-derived from the working state, because filling destroys the
evidence it rests on. A directed network whose asymmetry lies entirely in
its missing cells becomes symmetric as soon as those cells are initialised
at zero; a measure recomputed from that state would classify it as
undirected and, for instance, return the per-node reciprocity ratio as
missing for every node, quietly dropping a genuine predictor from that
visit.

Under tie-wise updating each unordered pair is visited once. Both
...
$p = \tfrac{1}{2}\left(\mathrm{logit}^{-1}(\eta_{ij}) +
\mathrm{logit}^{-1}(\eta_{ji})\right)$, and on the linear predictor scale
for a weighted one; the single resulting draw is written to both cells. The
two predictors generally differ even for an undirected target, because the
dyad-level design retains the sender/receiver-specific terms of the other
(possibly directed) networks and of the nodal attributes, so averaging
them, rather than arbitrarily privileging one direction, keeps the update
symmetric in the two nodes. Under the simultaneous scheme the mirror cells
are necessarily drawn from the same snapshot and can disagree; each
disagreeing pair is then reconciled after the draw, by keeping one of the
two values at random for a binary network --- each is an equally valid draw
of the same tie --- and by averaging them for a weighted one. Observed
cells are never overwritten by either step.
...
combining it with an explicit \code{net\_update = "gibbs"} is an error. Note of course that using random intercepts is only meaningful if missing ties are tie non-response and not actor non-response \citep{huisman2008}, since the latter means that all data for the random intercept of a missing node are missing.
...
class that respondents could not make): such cells are excluded from the initialisation donor pool,
and removed from the dyad-level regression rows, so design-zeros cannot
inflate the zeros of the model. \code{net\_dependence} declares
...
like structural cells for that visit (i.e., fixed to zero and removed from the estimation); and after every network
re-imputation the rules are followed, so every completed
network list satisfies every rule.

\paragraph{Attributes.} An attribute's predictor set includes the homophily block built from the
...
homophily or social influence, the composition of a node's alters on the
 variable being imputed (its mean alter value, and in particular the
...
the average-absolute-difference terms), since regressing a variable on
...
\code{method} may also be a named character vector assigning
\emph{per-target} methods, \pkg{mice}-style: named entries set the method
for individual attributes or networks, and at most one unnamed entry sets
the default for the rest --- e.g.\ \code{method = c("cart", performance =
"norm")} imputes \code{performance} by Bayesian linear regression and
everything else by trees. Entries naming a multinomial attribute, or a
network whose ties are updated tie-wise or with random intercepts, are
ignored with a message, since those targets' working models are fixed by
design.
...
relative to a modest number of nodes --- \code{netmice()} follows the same variable reductions as described above.
...
tie-wise updating, an interaction involving any endogenous term is, like those
main effects, re-evaluated from the current matrix state after every single
tie draw, so the interaction stays live throughout the sequential sweep. That
sweep is repeated \code{net\_sweeps} times per visit (default $5$), each pass
visiting the missing cells in a fresh random order and conditioning on every
tie drawn so far, with the working model fitted once per visit. The full lists of referenceable dyad-level
...
formula depends on are automatically protected from being dropped by
...
diagnostics, the mean and variance of each imputed attribute
...
iteration. Because those network statistics are computed on the
\emph{completed} network, observed ties included, they are diluted by the
missingness fraction and can look flat while the imputed portion drifts; the
mean and variance of the imputed ties alone are therefore tracked separately
and are the more sensitive diagnostic.

In addition, \code{netmice()} reports rank-normalized split-$\widehat{R}$
\citep{GelmanRubin1992,Vehtari2021} for each tracked quantity, computed after
discarding the first half of the iterations, and warns when any value exceeds
$1.05$. Constant traces return \code{NA} rather than a spurious statistic.
Users should note that $\widehat{R}$ is a large-sample diagnostic: at the
default \code{maxit}, the retained draws per half-chain number in the single
digits, and the statistic is correspondingly noisy in both directions.

\subsection{Data entry: matrices or an edgelist}
\label{sec:edgelist}

Adjacency matrices are rarely the form in which network data arrives, so
\code{networks} also accepts a single data frame \emph{edgelist}, described by
\code{edgelist\_options}. One edgelist may define many networks:
\code{edgelist\_format = "long"} splits it on the interaction of the columns
named in \code{edgelist\_split}, producing one network per observed
combination (so waves crossed with name-generator questions yield a network
per wave--question pair), while \code{"wide"} treats each named column as a
separate network holding that tie's value or \code{NA}.

Because an edgelist carries node names rather than positions, the \code{id}
argument names the column of \code{data} holding those identifiers; it is
required for edgelist input and optional, though advisable, for matrices,
where it matches and reorders their dimnames. Two further options carry the
information an edgelist structurally cannot. \code{nodelist} supplies each
network's roster, without which isolates and wholly non-responding actors
would be invisible; a node off a network's roster is treated as structurally
absent there rather than as having no ties. \code{missing} names the actors
whose ties are unknown --- by default their \emph{outgoing} ties, the ordinary
actor non-response case \citep{huisman2008} --- so that their unlisted cells
become missing rather than observed zeros. Nodes appearing in the edgelist but
not in \code{data} are added with all attributes missing, and reported.

The distinction matters because an edgelist records only which ties exist:
every unlisted cell is a zero by omission, and nothing in the format
distinguishes ``no tie'' from ``not asked''. \code{netmice()} therefore warns
when an actor is marked missing in a direction in which the edgelist
nonetheless lists ties for them, and directs the user to the wide format or to
matrices, which can express partial observation. The warning is
direction-matched: an actor whose outgoing ties are missing is still expected
to appear as a receiver, since others nominated them.
...
multinomial). Numbers below are the output of running this code with
...
\code{net\_measures()} directly (the `attributes' argument is only needed when
the homophily block is among the requested measures):
...
which predictors would enter each imputation model if only those predictors with
medium correlation with the target are considered. With only $n = 40$ nodes,
chance correlations of network measures easily exceed the \code{mincor} default
of $0.1$, so a strict threshold is used here purely for illustration:
...
\subsection{Specifying the imputation models}
\label{sec:congeniality}

When \code{models} is left unspecified, each imputation model is assembled
automatically from the available predictors, and \code{netmice()} says once per
session that supplying \code{models} is preferable. The reason is
congeniality \citep{Meng1994}: an imputation model must be at least as rich as
the analysis model that will be fitted to the completed data. Imputing without
an interaction, a non-linearity, or a network effect that the substantive
model will estimate draws the imputations from a distribution in which that
term is absent, biasing the pooled estimate toward the null however large $m$
is. For missing network data specifically, the consequences of an
under-specified imputation model have been documented by \citet{Krause2020}.
...
require non-negative edges, while others (the weighted reciprocity and
...
negative tie value. Extending the package to signed networks would
...
visit. Natural extensions include geometrically down-weighted shared-partner statistics in the spirit
...
network imputation is simple and efficient and likely sufficient in many
...
consuming and is thus not implemented or suggested here.

\textbf{Method tuning parameters.} \code{netmice()}'s \code{method}
argument dispatches generically to \pkg{mice}'s numeric-response
univariate methods, globally or per target
(Section~\ref{sec:netmice}), but the methods' tuning parameters
(\code{cart}'s \code{minbucket}, \code{rf}'s \code{ntree}, PMM's matching
type) currently keep \pkg{mice}'s defaults; exposing them --- per method,
or per target alongside the method itself --- is a natural next step, as
the internal dispatch is already method-agnostic.

\section{Summary}
\label{sec:summary}

\pkg{netimpute} provides a single, coherent \proglang{R} interface for a
problem that is otherwise split across separate tools, jointly imputing missing
